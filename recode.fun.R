recode.fun <- function(q, scale = 1){
  if(scale == 1){
    ...
    q.recoded = recode(q, "NOT a first-generation college student" = 0, "a first-generation college student" = 1 )
  }
  
  else if (scale == 10){
    q.recoded = recode(q, "Below 1.00" = 1, "1.00 – 1.99" = 2, "2.00 – 2.99" = 3,
                       "3.00 – 4.00" = 4, "Don’t know" = 5, "Prefer not to say" = 6)
  }
  
  else if (scale == 11){
    q.recoded = recode(q, "a Science, Technology, Engineering, or Mathematics (STEM) major" = 1,
                       "NOT a Science, Technology, Engineering, or Mathematics (STEM) major" = 0)
  }
  else if(scale == 12){
    q.recoded = recode(q, "Male" = 0, "Female" = 1, "Prefer not to say" = 2)
  }
  else if(scale == 13){
    q.recoded = recode(q, "Freshman" = 1, "Sophomore" = 2, "Junior" = 3, "Senior" = 4)
  }
  else if(scale == 14){
    q.recoded = recode(q, "Working Class" = 1, "Lower Middle Class" = 2, "Middle Class" = 3, "Upper Middle Class" = 4, "Weatlthy Class" = 5)
  }
  else if (scale == 15){
